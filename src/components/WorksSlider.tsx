...
type VisualVariant = 'zirconia' | 'emax' | 'navigation'
...
const slides: Slide[] = [
  { variant: 'zirconia', eyebrow: 'Направление', title: 'ZrO2', aria: 'Абстрактная визуализация керамических коронок ZrO2' },
  { variant: 'emax', eyebrow: 'Направление', title: 'IPS E.MAX', aria: 'Абстрактная визуализация коронки IPS E.MAX' },
  { variant: 'navigation', eyebrow: 'Направление', title: 'Навигационный протокол', aria: 'Абстрактная визуализация навигационного шаблона' },
]

function PortfolioVisual({ variant, aria }: { variant: VisualVariant; aria: string }) {
  ...
  return (
    <svg className="work-visual" viewBox="0 0 920 620" role="img" aria-label={aria} preserveAspectRatio="xMidYMid slice">
      ...
      <g opacity=".24" stroke="#6aa0bf" strokeWidth="1">
        ...
        <circle cx="174" cy="58" r="4" fill="#8fc5e4" stroke="none" /><circle cx="266" cy="136" r="4" fill="#8fc5e4" stroke="none" /><circle cx="106" cy="221" r="4" fill="#8fc5e4" stroke="none" />
      </g>
          ...
          <g transform="translate(220 126)">
            ...
            {[150, 265, 378].map((x) => (
              <g key={x} transform={`translate(${x} 115)`}>
                <ellipse cx="0" cy="0" rx="37" ry="25" fill="#07131e" opacity=".74" />
                <ellipse cx="0" cy="0" rx="25" ry="16" fill={`url(#${metal})`} />
                <ellipse cx="0" cy="0" rx="12" ry="8" fill="#0c1c28" />
        ...
        {slides.map((slide) => (
          <article className="work-slide" key={slide.variant}>
            <div className="work-slide__media">
              <PortfolioVisual variant={slide.variant} aria={slide.aria} />
            </div>
            <div className="work-slide__copy">
              <span className="eyebrow">{slide.eyebrow}</span>
              <h3>{slide.title}</h3>
              <p>Визуальная сцена, созданная для портфолио STLab.</p>
            </div>
          </article>
        ))}
      </div>
